...
FH_estimacion <- readRDS(
  "Modelo_area/PER_2024/ASFR/20-24/output/estimacion_ASFR_binomial_01102025.rds") 

FH_cadena <- readRDS(
  "Modelo_area/PER_2024/ASFR/20-24/output/Cadenas_thetapred_01102025.rds") 
...
FH_estimacion <- FH_estimacion %>% filter(! theta_pred > 0.507017) 

censo <- readRDS(
  "Modelo_area/PER_2024/ASFR/20-24/output/censo_muj_20_24.rds")

Direct_estimation_dam <- readRDS(
  "Modelo_area/PER_2024/ASFR/20-24/output/estimación_directa_dam.rds") %>%
...
R_dam2 <- temp %>% group_by(dam) %>% 
  summarise(
    R_dam_RB = unique(ASFR_dam) / sum((n  / dam_pp) * theta_pred)
  ) %>%
...
temp_draws <- FH_cadena %>% inner_join(temp %>% dplyr::select(provi, ASFR_dam, n, dam_pp, dam),
                                       by = "provi")
...
R_dam2_draws <- temp_draws %>% 
  ...
  summarise(
    ...
    across(.cols = starts_with("iter_"), 
           .names = "R_dam_RB_{.col}",
           .fns = ~ unique(ASFR_dam) / sum((n / dam_pp) * .x)
    )
  ) %>% 
  mutate(
...
saveRDS(estimacionesBench_draws,"Modelo_area/PER_2024/ASFR/20-24/output/estimacion_ASFR_bench_binomial_draws_01102025.rds" )
...
consolidado_temp <- temp_draws %>% 
  mutate(theta_reg_RB_draws = (theta_reg_RB_draws) * 1000,
         theta_pred_draws =  (theta_pred_draws) *1000 ,
         ASFR_dam = (ASFR_dam) *1000) 
...
ggsave(plot = a,
       filename =  "Modelo_area/PER_2024/ASFR/20-24/output/modelo_binomial_benchmarking_01102025.jpeg",
       scale = 3)
...
ASFR_final <- estimacionesBench_draws %>% 
  transmute(
    dam,
    provi,
    ASFR_cadenas = dplyr::select(., starts_with("iter_") & ends_with("_bench")),
    ASFR_media = rowMeans(ASFR_cadenas),
    ASFR_sd = apply(ASFR_cadenas, 1, sd)
  ) %>%
  transmute(
    dam,
    provi,
    ASFR_20_24 = ASFR_media*1000,
    ASFR_se = ASFR_sd*1000,
    ASFR_cv = ASFR_sd / ASFR_media*100,
    ASFR_LCI = apply(ASFR_cadenas, 1, quantile, probs = 0.025)*1000, 
    ASFR_UCI = apply(ASFR_cadenas, 1, quantile, probs = 0.975) *1000 
  ) 

saveRDS(
  ASFR_final,
  "Modelo_area/PER_2024/ASFR/20-24/output/estimacion_ASFR_bench_binomial_01102025.rds"
)
