...
base_MEF <- read.csv("Modelo_area/PER_2024/TFR/input/REC0111_2024.csv")# Modulo mujeres en edad fertil 12 - 49 años
base_emb <- read.csv("Modelo_area/PER_2024/TFR/input/REC21_2024.csv") #Historial de embarazos
...
base_MEF <- base_MEF %>% select(
  ...
  edad_actual, edad_grup, dam,dame, area, nivel_edu, anoest, fep_m, etnia,strata
)
base_MEF %>% group_by(id_individual) %>% tally()
summary(base_MEF)

base_MEF <- base_MEF %>% filter(!is.na(dam))

saveRDS(base_MEF, file ="Modelo_area/PER_2024/TFR/output/base_MEF.rds")
# --------------------------------------#
#            Births module              #
# --------------------------------------#
...
vars_mujer <- c("date_entrevista", "date_nac_muj", "fep_m", "upm", "strata",
                "dam", "dame", "area", "codigo_pais", "nivel_edu",
...
saveRDS(base_emb, file = "Modelo_area/PER_2024/TFR/output/base_nacimientos.rds")
