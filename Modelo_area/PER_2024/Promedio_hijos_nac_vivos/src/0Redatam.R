...
CONTEOS <- redatam.query(peru, "freq DISTRITO.REDCODE  by PERSONA.C5P02
                                  by PERSONA.C5P041
                                  by PERSONA.C5P02
                                  by PERSONA.C5P27",
                         tot.omit = FALSE)
...
censo_mrp <- CONTEOS2 %>%
  ...
  value) %>%
  group_by(dame, area, sexo, edad, etnia, discapacidad, anoest) %>%
  summarise(n = sum(value), .groups = "drop")
...
tasa_censo <- model.matrix(dame ~ -1 +.,
                           data = censo_mrp %>% dplyr :: select(-n)) %>% 
  data.frame() %>%
  mutate(dame = censo_mrp$dame,
         n = censo_mrp$n) %>% 
  group_by( dame) %>%
...
tasa_censo_m <- model.matrix(dame ~ -1 +.,
                             data = censo_mrp_m  %>% dplyr :: select(-n)) %>% 
  data.frame() %>%
  mutate(dame = censo_mrp_m$dame,
         n = censo_mrp_m$n) %>% 
  group_by(dame) %>%
  summarise_all(~weighted.mean(x = .,w = n)) %>%
  mutate(etnia1 = 1-etnia3-etnia2) %>% 
...
CONTEOS <- redatam.query(peru,
                         "freq DISTRITO.REDCODE  by VIVIENDA.VAREA
...
# Energía eléctrica ----------------------------------------------------
CONTEOS <- redatam.query(peru,
                         "freq DISTRITO.REDCODE  by PERSONA.C5P02  by VIVIENDA.VAREA
                          by VIVIENDA.C2P11",
                         tot.omit = FALSE)
...
####################################################
OCUPACION <- redatam.query(peru, "freq DISTRITO.REDCODE  by PERSONA.C5P02  by VIVIENDA.VAREA
                           by PERSONA.PET
                           ", tot.omit = FALSE)
...
OCUPACION2 <- OCUPACION2 %>%
  transmute(
    ...
    value
  ) %>% group_by(dame, ocupados, desocupados) %>%
...
CONTEOS <- redatam.query(
  peru,
  "freq DISTRITO.REDCODE  by PERSONA.C5P02 by VIVIENDA.VAREA by HOGAR.C3P205",
  tot.omit = FALSE
)
...
CONTEOS <- redatam.query(
  peru,
  "freq DISTRITO.REDCODE  by PERSONA.C5P02 by VIVIENDA.VAREA by HOGAR.C3P204",
  tot.omit = FALSE
)
...
CONTEOS <- redatam.query(
  peru,
  "freq DISTRITO.REDCODE  by PERSONA.C5P02 by VIVIENDA.VAREA by HOGAR.C3P209",
  tot.omit = FALSE
)
...
CONTEOS <- redatam.query(
  peru,
  "freq DISTRITO.REDCODE  by PERSONA.C5P02 by VIVIENDA.VAREA by HOGAR.C3P214",
  tot.omit = FALSE
)
...
CONTEOS <- redatam.query(
  peru,
  "freq DISTRITO.REDCODE  by PERSONA.C5P02 by VIVIENDA.VAREA by HOGAR.C3P15",
  tot.omit = FALSE
)
...
CONTEOS <- redatam.query(peru,
                         "freq DISTRITO.REDCODE  by PERSONA.C5P02 
                          by PERSONA.SITUEMP",
                         tot.omit = FALSE)
SITUEMPL <- CONTEOS %>% 
  filter_at(vars(matches("_label")),
...
hijos_nacidos <- hijos_nacidos %>%
  ...
  value)%>%
  filter(edad >= 15 & edad <= 49 & sexo == 2) %>% group_by(dame)%>%
  summarise(hijos_nacidos = sum(value*hijos_nacidos), .groups = "drop")

Prop_hijos <- hijos_nacidos %>% left_join(muj_15_49, by = "dame") %>% 
...
saveRDS(statelevel_predictors_df, "Modelo_area/PER_2024/Promedio_hijos_nac_vivos/input/statelevel_predictors_df_update_dame.rds")
...
saveRDS(statelevel_predictors_df_m, "Modelo_area/PER_2024/Promedio_hijos_nac_vivos/input/statelevel_predictors_df_update_dame_muj.rds")
