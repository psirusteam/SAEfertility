...
p1<-ggplot(est_dir_FGV,aes(x=hijos_nacidos,y=ln_sigma2))+
  ...
  xlab("Promedio de hijos vivos por distrito")
#tamaño muestral versus ln_sigma2
p2<-ggplot((est_dir_FGV),aes(x=nd,y=ln_sigma2))+
  geom_point()+geom_smooth(method="loess")+
  xlab("Tamaño de la muestra por distrito")
...
ggsave(plot = fgv_graf,
       filename =  "Modelo_area/PER_2024/Promedio_hijos_nac_vivos/output/FGV_prom_hij_n_v.jpeg", 
       scale = 3)


#Guardar la base con FGV  
saveRDS(est_dir_FGV, "Modelo_area/PER_2024/Promedio_hijos_nac_vivos/output/estimación_directa_phv_FGV.rds")
