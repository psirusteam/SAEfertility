...
est_dir <- readRDS(
  "Modelo_area/PER_2024/CMR/output/estimación_directa_provi_CMR.rds"
)
...
info_covariables <- readRDS("Modelo_area/PER_2024/CMR/input/statelevel_predictors_df_update_provi.rds") %>% mutate(dam = str_sub(provi, 1, 2))

info_satelitales <- readRDS("Modelo_area/PER_2024/CMR/input/satelitales_provincia.rds") %>%
  rename(
    provi = provi,
...
# # Estandarización para media 0 y desviación 1 ----------------------------------
# 
# vars <- names(info_covariables)
# vars <- vars[!(vars %in% c("provi", "depto"))]
# 
# stan_params <- info_covariables %>% 
...
#     sigma <- sd(., na.rm = TRUE)
#     
#     # Aplicar Z-Score. Si la DE es cero (variable constante), el valor es 0.
...
#Dominios observados
data_dir <- base_FH %>% filter(!is.na(CMR))
...
# nombres <- nombres[!(nombres %in% c("provi", "dam","depto","dam_25","area1","anoest2",
#                                     "anoest3","anoest4","rezago_escolar", "analfabeta",
#                                     "tasa_cuent_prop","tasa_empleado"
# ))]
...
y_effect  = ceiling((data_dir$CMR_2)*n_effec)
...
saveRDS(object = model_FH_Binomial,
        file = "Modelo_area/PER_2024/CMR/output/model_binomial_sobrevivencia_30092025_2.rds")

model_FH_Binomial <- readRDS(file = "Modelo_area/PER_2024/CMR/output/model_binomial_sobrevivencia_30092025_2.rds")
...
ppc_dens_overlay(y = as.numeric(data_dir$CMR_2), y_pred2)


#Analisis del grafico de la convergencia de las cadenas de sigma cuadrado_u
posterior_sigma2_u <- as.array(model_FH_Binomial, pars = "sigma_u")
...
saveRDS(object = posterior_theta,
        file = "Modelo_area/PER_2024/CMR/output/Cadenas_thetapred_30092025.rds")
...
ggsave(plot = p_comb,
       filename =  "Modelo_area/PER_2024/CMR/output/modelo_beta_efectos_aleatorios.jpeg",
       scale = 5)


#Guardar la base de las estimaciones
saveRDS(data_estimation, "Modelo_area/PER_2024/CMR/output/estimacion_CMR_FH_binomial_30092025.rds")
