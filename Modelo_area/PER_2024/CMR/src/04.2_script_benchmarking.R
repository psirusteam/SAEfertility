...
FH_estimacion <- readRDS(
  "Modelo_area/PER_2024/CMR/output/estimacion_CMR_FH_binomial_30092025.rds") 

FH_cadena <- readRDS(
  "Modelo_area/PER_2024/CMR/output/Cadenas_thetapred_30092025.rds") 
...
FH_estimacion <- FH_estimacion %>% filter(! theta_pred > 0.02959) 

censo <- readRDS(
  "Modelo_area/PER_2024/CMR/output/censo_hijos_nacidos.rds")

Direct_estimation_dam <- readRDS(
  "Modelo_area/PER_2024/CMR/output/estimación_directa_dam.rds") %>%
...
R_dam2 <- temp %>% group_by(dam) %>% 
  summarise(
    R_dam_RB = unique(CMR_dam) / sum((hijos_nacidos  / dam_pp) * theta_pred)
  ) %>% mutate(
    R_dam_RB = case_when(
      is.nan(R_dam_RB) ~ 1,
      is.infinite(R_dam_RB) ~ 1,
      TRUE ~ R_dam_RB  
    )
  ) %>% 
...
temp_draws <- FH_cadena %>% inner_join(temp %>% dplyr::select(provi, CMR_dam, hijos_nacidos, dam_pp, dam),
                                      by = "provi")
...
R_dam2_draws <- temp_draws %>% 
  ...
  summarise(
    ...
    across(.cols = starts_with("iter_"), 
           .names = "R_dam_RB_{.col}",
           .fns = ~ unique(CMR_dam) / sum((hijos_nacidos / dam_pp) * .x)
    )
  ) %>% 
  mutate(
...
saveRDS(estimacionesBench_draws,"Modelo_area/PER_2024/CMR/output/estimacion_CMR_bench_binomial_draws_30092025.rds" )
...
ggsave(plot = a,
       filename =  "Modelo_area/PER_2024/CMR/output/modelo_binomial_benchmarking_30092025.jpeg",
       scale = 3)

CMR_final <- estimacionesBench_draws %>% 
  transmute(
    dam,
    provi,
    CMR_cadenas = dplyr::select(., starts_with("iter_") & ends_with("_bench")),
    CMR_media = rowMeans(CMR_cadenas),
    CMR_sd = apply(CMR_cadenas, 1, sd)
  ) %>%
  transmute(
    dam,
    provi,
    CMR_media = CMR_media*1000,
    CMR_se = CMR_sd*1000,
    CMR_cv = CMR_sd / CMR_media*100,
    CMR_LCI = apply(CMR_cadenas, 1, quantile, probs = 0.025)*1000, 
    CMR_UCI = apply(CMR_cadenas, 1, quantile, probs = 0.975) *1000 
  ) 
