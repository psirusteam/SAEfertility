...
base_MEF <- read.csv("Modelo_area/PER_2024/CMR/input/REC0111_2024.csv") # Modulo mujeres en edad fertil 12 - 49 años
...
base_emb <- read.csv("Modelo_area/PER_2024/CMR/input/REC21_2024.csv") #Historial de embarazos
...
for (i in seq_along(dam)) {
  ...
  df <- as.data.frame(out) %>%
    ...
    filter(startsWith(indicador, "CMR")) %>%   # solo CMR
    mutate(dam = a)
...
for (i in seq_along(dame)) {
  ...
  df <- as.data.frame(out) %>%
    rownames_to_column("indicador") %>%
    filter(startsWith(indicador, "CMR")) %>%   # solo U5MR
    mutate(dame = a)
...
base_sae <- estimacion %>% data.frame()%>%
  ...
  transmute(
    ...
    CMR = R,              # Estimación de la variable
...
saveRDS(base_sae, "Modelo_area/PER_2024/CMR/output/estimación_directa_distrito_CMR.rds")
