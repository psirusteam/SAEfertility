...
FH_estimacion <- readRDS(
  "Modelo_area/PER_2024/U5MR/output/estimacion_U5MR_FH_binomial_28092025.rds") 

FH_cadena <- readRDS(
  "Modelo_area/PER_2024/U5MR/output/Cadenas_thetapred_28092025.rds") 
...
FH_estimacion <- FH_estimacion %>% filter(! theta_pred > 0.15156) 
...
R_dam2 <- temp %>% group_by(dam) %>% 
  summarise(
    R_dam_RB = unique(U5MR_dam) / sum((hijos_nacidos  / dam_pp) * theta_pred)
  ) %>%
...
temp_draws <- FH_cadena %>% inner_join(temp %>% dplyr::select(provi, U5MR_dam, hijos_nacidos, dam_pp, dam),
                                       by = "provi")
...
R_dam2_draws <- temp_draws %>% 
  ...
  summarise(
    ...
    across(.cols = starts_with("iter_"), 
           .names = "R_dam_RB_{.col}",
           .fns = ~ unique(U5MR_dam) / sum((hijos_nacidos / dam_pp) * .x)
    )
  ) %>% 
  mutate(
...
saveRDS(estimacionesBench_draws,"Modelo_area/PER_2024/U5MR/output/estimacion_U5MR_bench_binomial_draws_28092025.rds" )
...
ggsave(plot = a,
       filename =  "Modelo_area/PER_2024/U5MR/output/modelo_binomial_benchmarking_28092025.jpeg",
       scale = 3)
...
saveRDS(
  estimacionesBench,
  "Modelo_area/PER_2024/U5MR/output/estimacion_U5MR_bench_binomial_28092025.rds"
)
