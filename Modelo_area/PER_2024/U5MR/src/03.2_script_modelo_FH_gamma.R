...
nombres <- nombres[!(nombres %in% c("dame", "dam","depto", "area1", "tasa_empleador_m","dam_25",
                                    "anoest4", "rezago_escolar_m", "carece_computadora_m", 
                                    "analfabeta_m", "mean_hijos"))]
...
fit_FH_gamma <- "Modelo_area/PER_2024/0funciones/19_FH_gamma.stan"
...
sample_data <- list(
  ...
  y  = as.numeric(data_dir$U5MR_2), # Estimación directa
  V = data_dir$vardir  
)
...
saveRDS(object = model_FH_gamma,
        file = "Modelo_area/PER_2024/U5MR/output/model_FH_gamma.rds")

y_pred_B <- as.array(model_FH_gamma, pars = "theta") %>% 
  as_draws_matrix()
...
posterior_sigma2_u <- as.array(model_FH_gamma, pars = "sigma_u")
(mcmc_dens_chains(posterior_sigma2_u) +
    mcmc_areas(posterior_sigma2_u) ) / 
  mcmc_trace(posterior_sigma2_u)
validacionRhat<-summary(model_FH_gamma)$summary%>%as.data.frame()
mcmc_rhat(validacionRhat$Rhat)
validacionRhat%>%filter(Rhat>1.1) #aumentar iteraciones, ideal Rhat<1.1

posterior_beta <- as.array(model_FH_gamma, pars = "beta")

mcmc_trace(posterior_beta)

#metodo de validacion del modelo FH obtenido en STAN
theta_obs <-   summary(model_FH_gamma, pars =  "theta")$summary %>%
  data.frame()
...
data_dir %<>% mutate(
  thetadir = U5MR,
  theta_pred = theta_obs$mean,
  theta_pred_EE = theta_obs$sd,
...
theta_nobs <-   summary(model_FH_gamma, pars =  "thetapred")$summary %>%
  data.frame()

data_syn <- data_syn %>% 
  mutate(
    theta_pred = theta_nobs$mean,
    theta_pred_EE = theta_nobs$sd,
    Cv_theta_pred = theta_pred_EE/theta_pred,
    doble_theta_pred_EE = theta_pred_EE*2)
...
options(scipen=999)
summary(data_estimacion$Cv_theta_pred)

#Guardar la base de las estimaciones
saveRDS(data_estimacion, "Modelo_area/PER_2024/U5MR/output/estimacion_U5MR_FH_gamma.rds")
